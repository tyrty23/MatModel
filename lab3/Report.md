...
```math
Примем, что $V_0=0$. Для первой ступени $V_{p_1}=V_0+V_г*ln(\frac{m_0}{m(t)})=V_г*ln(\frac{m_0}{m(t)})$, где $m_0=m_p+m_{г1}+m_{г2}+m_n$. Пусть топливо в первой ступени израсходавоно, тогда максимальная скорость для перввой ступени будет $V_{p1_{max}}=V_г*ln(\frac{m_0}{m_p+m_{г2}+m_н})$. 

Для второй ступени $V_{p2}=V_{p1_{max}} + V_г*ln(\frac{m_p+m_{г2} + m_н}{m_{1}(t)})$ и $V_{p2_{max}}=V_{p1_{max}} + V_г*ln(\frac{m_p+m_{г2} + m_н}{m_p+m_н})$
```
