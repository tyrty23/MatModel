...
Рассмотрим движение двухступенчатой ракеты (для трехступенчатой аналогично). Примем, что $( V_0 = 0 \)$.

Для первой ступени скорость рассчитывается по формуле:
$
V_{p_1} = V_0 + V_g \ln\left(\frac{m_0}{m(t)}\right) = V_g \ln\left(\frac{m_0}{m(t)}\right),
$
где \( m_0 = m_p + m_{г1} + m_{г2} + m_n \).

После расходования топлива в первой ступени максимальная скорость будет равна:
$
V_{p1_{max}} = V_g \ln\left(\frac{m_0}{m_p + m_{г2} + m_n}\right).
$

Для второй ступени скорость определяется следующим образом:
$
V_{p2} = V_{p1_{max}} + V_g \ln\left(\frac{m_p + m_{г2} + m_n}{m_{1}(t)}\right),
$
а максимальная скорость второй ступени будет:
$
V_{p2_{max}} = V_{p1_{max}} + V_g \ln\left(\frac{m_p + m_{г2} + m_n}{m_p + m_n}\right).
$
