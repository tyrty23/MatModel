...
Рассмотрим движение двухступенчатой ракты. Для первой ступени ракета движется по уравнению:
$V_{p1} = V_0 + V_г*ln(\frac{m_0}{m(t)})$
где $m_0 = m_p + m_{г1} + m_{г2} + m_n$
После сжигания топлива первой ступени максимальная скорость будет определяться уравнением:
$V_{p1_{max}} = V_г*ln(\frac{m_0}{m_p + m_{г2} + m_n})$

Для второй ступени скорость определяется как:
$V_{p2} =V_{p1_{max}} + V_г*ln(\frac{m_p + m_{г2} + m_n}{m_1(t)})$
и максимальная скорость для второй ступени:
$V_{p2_{max}} =V_{p1_{max}}+ V_г*ln(\frac{m_p + m_{г2} + m_n}{m_p + m_n})$

Для расчета дальности полета ракеты мы используем уравнение, в котором учитываются начальная скорость, угол полета и начальная высота:

$R=V_0*cos(\theta) * (\frac{V_0*sin(\theta) + \sqrt{(V_0*sin(\theta))^2 + 2gh_0}}{g})$
...
$R=V_0*cos(\theta) * (\frac{V_0*sin(\theta) + \sqrt{(V_0*sin(\theta))^2 + 2gh_0}}{g}) =>[м] = [м/c] *[1] *([м/c] *[1] + \sqrt{([м/c] *[1])^2 + [м]*[м/с^2]}) / [м/с^2] = [м]$ 
